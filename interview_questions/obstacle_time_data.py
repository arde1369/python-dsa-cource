...
"""
"""
2) We would like to implement a new function in RunCollection called "best_of_bests". This is a measure of how fast a run could be if everything went perfectly, 

and is determined by taking the fastest times for each obstacle across all runs (even incomplete ones) and summing them.
...
class Run:
    ...
    def __init__(self, course):
        ...
        self.complete = False  # a boolean, True if the run is a full run of the
                               # course. This may be False if a run is in progress
                               # or aborted
        self.obstacle_times = [] # list of int, the times it took to complete each
                                 # obstacle
    ...
    def add_obstacle_time(self, obstacle_time):
        ...
        if self.complete: raise ValueError("Cannot add obstacle to complete run")
...
class RunCollection(object):
    ...
    def personal_best(self):
        ...
        for run in self.runs:
            if run.complete:
                minRun = min(minRun, run.get_run_time())
        return minRun
        #Fix is above this line

        #return min(run.get_run_time() for run in self.runs) <-- was causing the bug
    """
    we'll take each column in the 4x4 array and turn it into a array
    sort said array in asc order
    add array[0] across those newly made arrays
    [3, 4, 5, 6],
    [4, 4, 4, 5],
    [4, 5, 4, 6],
    [5, 5, 3]
